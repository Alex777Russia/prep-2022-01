...
enum RETURN_ERR_CODES {
    ERR_PTR_VAL = -1,
    ERR_BAD_INPUT_PARAMETERS = -3,
};
...
int det(const Matrix* matrix, double* val) {
    ...
    if (matrix->rows != matrix->cols) {
        return ERR_BAD_INPUT_PARAMETERS;
    }
    ...
    for (size_t i = 0; i < adj_matrix->rows; ++i) {
        for (size_t j = 0; j < adj_matrix->cols; ++j) {
            ...
            adj_matrix->array[i * adj_matrix->rows + j] = sign *
                    det_counting(smaller_matrix, smaller_matrix->rows);
...
Matrix* inv(const Matrix* matrix) {
    ...
    Matrix* invariant_matrix = mul_scalar(adj_matrix, val);
    free_matrix(adj_matrix);

    return invariant_matrix;
}
